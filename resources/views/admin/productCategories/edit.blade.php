...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.product-categories.update", [$productCategory->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="seo_name">{{ trans('cruds.productCategory.fields.seo_name') }}</label>
                <input class="form-control {{ $errors->has('seo_name') ? 'is-invalid' : '' }}" type="text" name="seo_name" id="seo_name" value="{{ old('seo_name', $productCategory->seo_name) }}">
                @if($errors->has('seo_name'))
                    <span class="text-danger">{{ $errors->first('seo_name') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.productCategory.fields.seo_name_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="seo_desc">{{ trans('cruds.productCategory.fields.seo_desc') }}</label>
                <input class="form-control {{ $errors->has('seo_desc') ? 'is-invalid' : '' }}" type="text" name="seo_desc" id="seo_desc" value="{{ old('seo_desc', $productCategory->seo_desc) }}">
                @if($errors->has('seo_desc'))
                    <span class="text-danger">{{ $errors->first('seo_desc') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.productCategory.fields.seo_desc_helper') }}</span>
            </div>
            <div class="form-group">
