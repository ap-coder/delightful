...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.content-pages.update", [$contentPage->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="page_sections">{{ trans('cruds.contentPage.fields.page_sections') }}</label>
                <div style="padding-bottom: 4px">
                    <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                    <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                </div>
                <select class="form-control select2 {{ $errors->has('page_sections') ? 'is-invalid' : '' }}" name="page_sections[]" id="page_sections" multiple>
                    @foreach($page_sections as $id => $page_sections)
                        <option value="{{ $id }}" {{ (in_array($id, old('page_sections', [])) || $contentPage->page_sections->contains($id)) ? 'selected' : '' }}>{{ $page_sections }}</option>
                    @endforeach
                </select>
                @if($errors->has('page_sections'))
                    <span class="text-danger">{{ $errors->first('page_sections') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.contentPage.fields.page_sections_helper') }}</span>
            </div>
            <div class="form-group">
