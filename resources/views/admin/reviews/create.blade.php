    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.reviews.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('service_reviewed') ? 'is-invalid' : '' }}" name="service_reviewed_id" id="service_reviewed_id">
                    @foreach($service_revieweds as $id => $service_reviewed)
                        <option value="{{ $id }}" {{ old('service_reviewed_id') == $id ? 'selected' : '' }}>{{ $service_reviewed }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product_reviewed') ? 'is-invalid' : '' }}" name="product_reviewed_id" id="product_reviewed_id">
                    @foreach($product_revieweds as $id => $product_reviewed)
                        <option value="{{ $id }}" {{ old('product_reviewed_id') == $id ? 'selected' : '' }}>{{ $product_reviewed }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('review_author') ? 'is-invalid' : '' }}" type="text" name="review_author" id="review_author" value="{{ old('review_author', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('review_content') ? 'is-invalid' : '' }}" type="text" name="review_content" id="review_content" value="{{ old('review_content', '') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('rating') ? 'is-invalid' : '' }}" type="number" name="rating" id="rating" value="{{ old('rating', '0') }}" step="1" required>
